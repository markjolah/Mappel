...
template<class Model>
ReturnIfSubclassT<std::unique_ptr<estimator::Estimator<Model>>, Model, PoissonNoise2DObjective>
make_estimator(Model &model, std::string ename)
{
    ...
    if (istarts_with(name,"Heuristic")) {
        ...
        return make_unique<estimator::CGaussHeuristicEstimator<Model>>(model);
    } else if (istarts_with(name,"CGauss")) {
        return make_unique<estimator::CGaussMLE<Model>>(model);
