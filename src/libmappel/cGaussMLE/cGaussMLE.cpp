...
void MLEFit(const float data[], const float PSFSigma, const int size,
    ...
    * \param Parameters array of fitting parameters to return (theta)
...
//         printf("Step#%i\n",kk);
//         printf("Theta(%i):[",kk);
//         for(ll=0;ll<NV;ll++) printf("%g,",theta[ll]);
//         printf("]\n");
//         printf("Numer:[");
//         for(ll=0;ll<NV;ll++) printf("%g,",NR_Numerator[ll]);
//         printf("]\n");
//         printf("Denom:[");
//         for(ll=0;ll<NV;ll++) printf("%g,",NR_Denominator[ll]);
//         printf("]\n");
//         printf("Step:[");
//         for(ll=0;ll<NV;ll++) printf("%g,",-NR_Numerator[ll]/NR_Denominator[ll]);
//         printf("]\n");
// 
//         // The update
//         printf("SacledStep:[");
        ...
        if (kk<2) {
            for (ll=0;ll<NV;ll++) {
                step=-gamma[ll]*min(max(NR_Numerator[ll]/NR_Denominator[ll], -maxjump[ll]), maxjump[ll]);
//                 printf("%g,",step);
                theta[ll]+=step;
            }
        } else {
            for (ll=0;ll<NV;ll++) {
                step=-min(max(NR_Numerator[ll]/NR_Denominator[ll], -maxjump[ll]), maxjump[ll]);
//                 printf("%g,",step);
                theta[ll]+=step;
            }
        }
//         printf("]\n");
        ...
        theta[2]=max(theta[2], 1e0f);
        theta[3]=max(theta[3], 1e-2f);
//         printf("Theta(%i):[",kk+1);
//         for(ll=0;ll<NV;ll++) printf("%g,",theta[ll]);
//         printf("]\n");
    ...
    for (kk=0;kk<NV;kk++) Parameters[kk]=theta[kk];
...
//*******************************************************************************************
void MLEFit_sigma(const float data[], const float PSFSigma, const int size, const int iterations,
        float Parameters[], float CRLB[], float *LogLikelihood){
    /*!
    * \brief basic MLE fitting kernel.  No additional parameters are computed.
    * \param data array of subregions to fit copied to GPU
    * \param PSFSigma sigma of the point spread function
    ...
    * \param Parameters array of fitting parameters to return for each subregion
