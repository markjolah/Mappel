...
# which the random number generator is seeded, and a prior samle of size Nsample is
...
Nsample = 80 #Number of samples for to test a vector operation with

@hypothesis.given(seed=npst.arrays(shape=1,dtype="uint64"))
def test_sample_prior_singla(model, seed):
...
def test_sample_prior(model, seed):
    """Check model.sample_prior and model.set_rng_seed repeatability"""
    theta = draw_prior_theta(model,seed,Nsample)
    theta2 = draw_prior_theta(model,seed,Nsample)
    assert np.all(theta == theta2), "Samples drawn with same seed should be identical"
    theta3 = model.sample_prior(Nsample)
    assert np.any(theta2 != theta3), "Successive samples should differ."
...
def test_simulate_image(model, seed):
    ...
    theta = draw_prior_theta(model,seed)
    im = model.simulate_image(theta,Nsample)
    check_image(model,im,Nsample)
...
def test_objective_llh(model,seed):
    ...
    assert llh_nn.size == Nsample
    assert llh_n1.size == Nsample
    assert llh_1n.size == Nsample
...
def test_objective_rllh(model,seed):
    ...
    assert rllh_nn.size == Nsample
    assert rllh_n1.size == Nsample
    assert rllh_1n.size == Nsample
...
def test_objective_grad(model,seed):
    ...
    check_grad(model, grad_nn, Nsample)
    check_grad(model, grad_n1, Nsample)
    check_grad(model, grad_1n, Nsample)
...
def test_objective_hessian(model,seed):
    ...
    check_hess(model, hess_nn, Nsample)
    check_hess(model, hess_n1, Nsample)
    check_hess(model, hess_1n, Nsample)
    check_hess(model, hess_11, 1)
...
def test_objective_negative_definite_hessian(model,seed):
    ...
    chol = [ scipy.linalg.cholesky(-hess_nn[...,n]) for n in range(Nsample)]
...
def test_expected_information(model,seed):
    ...
    check_hess(model,fisherI,Nsample) # fisherI should act like a hessian matrix (or stack of hessians)
...
def test_observed_information(model,seed):
    ...
    check_hess(model,obsI,1) # fisherI should act like a hessian matrix (or stack of hessians)
