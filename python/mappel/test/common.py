...
#def check_theta(model, theta):
    #assert theta.ndim == 1, "Incorrect theta dimension"
    #assert theta.shape[0] == model.num_params, "Incorrect number of params"
    #assert np.all(theta>0)
    #assert np.all(np.isfinite(theta))
    #assert model.theta_in_bounds(theta)


def check_theta(model, thetas, N=0):
    if N==0 or N==1:
...
def check_hess(model, hess, N=0):
    if N==0 or N==1:
        assert hess.ndim == 3 or hess.ndim == 2, "Incorrect hessian stack dimension"
    else:
        assert hess.ndim == 3, "Incorrect hessian stack dimension"
    assert hess.shape[0] == model.num_params, "Incorrect hessian shape"
    assert hess.shape[1] == model.num_params, "Incorrect hessian shape"
    if N==1:
        assert hess.ndim == 2 or hess.shape[2] == 1, "Incorrect number of columns"
    elif N>1:
        assert hess.shape[2] == N, "Incorrect number of columns"
    assert np.all(np.isfinite(hess))
    if hess.ndim == 2:
        assert np.all(hess == np.transpose(hess)), "Hessian should be symmetric"
    else:
        assert np.all(hess == np.transpose(hess,axes=[1,0,2])), "Hessian should be symmetric"

#def check_image(model,im):
    #"""Check a single returned image is the right shape and value."""
    #assert im.ndim == model.num_dim, "Image incorrect ndim"
    #assert all(im.shape == model.size), "Image incorrect size"
...
def check_image(model,im, N=0):
    ...
    if model.num_dim==1:
        ...
        assert np.all(im.shape[0:-1] == model.size), "Image stack incorrect base size"
    if N==1:
